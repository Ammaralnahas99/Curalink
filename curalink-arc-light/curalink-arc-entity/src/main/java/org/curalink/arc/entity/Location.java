...
@NamedQueries({
        ...
        @NamedQuery(name = Location.FIND_BY_STATUS_CREATED_BEFORE,
                query = "select new org.curalink.arc.entity.Location$LocationWithUIDs(" +
        ...
        @NamedQuery(name = Location.STATUS_COUNTS_BY_STORAGE_ID,
                query = "select new org.curalink.arc.entity.Location$StatusCounts(l.status, count(l)) " +
                        "from Location l " +
                        "where l.storageID = ?1 and l.status != org.curalink.arc.conf.LocationStatus.OK " +
                        "group by l.status")
})
@NamedNativeQueries({
