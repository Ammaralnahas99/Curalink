import { AppService, ChangeDetectionStrategy, ChangeDetectorRef, Component, Directive, ElementRef, EventEmitter, FactoryTarget, Globalvar, Host, HttpClient, Inject, Injectable, InjectionToken, Injector, Input, J4careHttpService, KeycloakService, MatCommonModule, NgModule, NgTemplateOutlet, Optional, Output, Pipe, Renderer2, Router, RuntimeError, Self, SkipSelf, Subject, Version, ViewChild, ViewEncapsulation, __decorate, __spreadProps, __spreadValues, _animationsDisabled, booleanAttribute, catchError, cloneDeep_default, computed, core_exports, forkJoin, forwardRef, from, getDOM, get_default, hasIn_default, inject, isPromise, isSubscribable, j4care, map, numberAttribute, of, shareReplay, signal, switchMap, untracked, ɵɵngDeclareClassMetadata, ɵɵngDeclareComponent, ɵɵngDeclareDirective, ɵɵngDeclareFactory, ɵɵngDeclareInjectable, ɵɵngDeclareInjector, ɵɵngDeclareNgModule } from "./chunk-4EEB53CS.js";
...
var User = class {
  ...
  set tokenParsed(value) {
    this._tokenParsed = value;
  }
};

// src/app/app-requests.service.ts
var _a;
var AppRequestsService = (_a = class {
  ...
  constructor($http, appService, $httpClient) {
    this.$http = $http;
  ...
  getDeviceInfo(dicomDeviceName, url) {
  ...
  getDeviceNameFromURL(url) {
    return this.deviceNameRequest$(url).pipe(shareReplay(1), catchError(error => {
      return of({
        dicomDeviceName: "NOT_FOUND"
      });
    }));
  }
  dcm4cheeArcRequest$() {
  ...
  getDcm4cheeArc() {
    ...
    if (this._dcm4cheeArcConfig) {
      ...
      return this.dcm4cheeArcRequest$().pipe(map(dcm4cheeArc => {
        tempDcm4cheeArch = dcm4cheeArc;
        if (hasIn_default(dcm4cheeArc, "dcm4chee-arc-urls[0]")) {
          this.baseUrl = get_default(dcm4cheeArc, "dcm4chee-arc-urls[0]");
          this.appService.baseUrl = this.baseUrl;
        }
        return dcm4cheeArc;
      }), switchMap(dcm4cheeArc => {
        let services = get_default(dcm4cheeArc, "dcm4chee-arc-urls").map(url => {
          return this.getDeviceNameFromURL(url).pipe(shareReplay(1), catchError(error => of(error)));
        });
        ...
        try {
          ...
          tempDcm4cheeArch["dcm4chee-arc-urls"].forEach((url, i) => {
            deviceNameUrlMap[url] = get_default(res, `${i}.dicomDeviceName`);
            if (!environment.production) {
              if (i === 1) deviceNameUrlMap[url] = `stowrsd`;
            }
            if (i > 0) {
  ...
  getMyDevice() {
    if (hasIn_default(this.appService.global, "myDevice")) {
      return of(this.appService.global.myDevice);
    } else {
      let deviceName;
      let archiveDeviceName;
      return this.getDcm4cheeArc().pipe(switchMap(res => {
        deviceName = get_default(res, "UIConfigurationDeviceName") || get_default(res, "dicomDeviceName");
        archiveDeviceName = get_default(res, "dicomDeviceName");
        return this.devicesRequest$(deviceName);
      }), map(res => {
        try {
          let global = cloneDeep_default(this.appService.global) || {};
          global["uiConfig"] = get_default(res, "dcmDevice.dcmuiConfig[0]");
          global["myDevice"] = res;
          ...
          this.appService.showError("Permission not found!");
        }
        return res;
      }));
    }
  ...
  getUiConfig() {
    if (hasIn_default(this.appService.global, "uiConfig")) {
      return of(this.appService.global.uiConfig);
    } else {
      return this.getMyDevice().pipe(map(res => {
        return get_default(res, "dcmDevice.dcmuiConfig[0]");
      }));
    }
  }
  getMyWebApps() {
    if (hasIn_default(this.appService.global, "myDevice")) {
      return of(get_default(this.appService.global.myDevice, "dcmDevice.dcmWebApp").map(dcmWebApp => {
        dcmWebApp.dcmKeycloakClientID = get_default(this.appService.global.myDevice, "dcmDevice.dcmKeycloakClient").filter(keycloakClient => {
      ...
      return this.getMyDevice().pipe(map(res => {
        return get_default(res, "dcmDevice.dcmWebApp").map(dcmWebApp => {
          dcmWebApp.dcmKeycloakClientID = get_default(this.appService.global.myDevice, "dcmDevice.dcmKeycloakClient").filter(keycloakClient => {
            return keycloakClient.dcmKeycloakClientID === dcmWebApp.dcmKeycloakClientID;
          })[0];
          return dcmWebApp;
...
AppRequestsService = __decorate([Injectable({
  providedIn: "root"
})], AppRequestsService);

// src/app/helpers/permissions/permission.service.ts
var _a2;
var PermissionService = (_a2 = class {
  constructor($http, mainservice, router, _keycloakService, appRequest) {
    this.$http = $http;
  ...
  getPermission(url) {
    if (this.mainservice.user && this.mainservice.user.roles) {
    ...
    } else return this.getConfigWithUser(() => {
      if (this.mainservice.user && !this.mainservice.user.user && this.mainservice.user.roles && this.mainservice.user.roles.length === 0) {
        ...
        if (this.mainservice.user && this.mainservice.user.su) return true;else return this.checkMenuTabAccess(url);
      }
    });
  }
  checkSuperAdmin(url) {
    if (this.mainservice.user && this.mainservice.user.su || !this.mainservice.user.user && this.mainservice.user.roles.length === 0) return true;else return this.getConfig(() => {
  ...
  getConfig(response) {
    let deviceName;
    let archiveDeviceName;
    if (!this.uiConfig && !this.configChecked) return this.appRequest.getDcm4cheeArc().pipe(switchMap(res => {
      ...
      return response.call(this);
    }));else return response.call(this);
  }
  getConfigWithUser(response) {
    ...
    if (!this.uiConfig && !this.configChecked) {
      return this._keycloakService.getUserInfo().pipe(map(user => {
        userInfo = user;
      }), switchMap(res => this.appRequest.getDcm4cheeArc()), map(dcm4cheeArc => {
        if (userInfo) {
          const roles = get_default(userInfo, "tokenParsed.realm_access.roles");
          let user = new User({
            authServerUrl: userInfo.authServerUrl,
            realm: userInfo.realm,
            user: get_default(userInfo, "userProfile.username"),
            roles,
            su: hasIn_default(dcm4cheeArc, "super-user-role") && roles.indexOf(get_default(dcm4cheeArc, "super-user-role")) > -1,
        ...
        try {
          ...
          if (this.mainservice.global.notSecure || this.mainservice.user && !this.mainservice.user.user && this.mainservice.user.roles && this.mainservice.user.roles.length === 0) {
            this.mainservice.global.notSecure = true;
          } else this.mainservice.showError("Permission not found!");
  ...
  checkMenuTabAccess(url) {
    if (hasIn_default(this.mainservice, "global.notSecure") && this.mainservice.global.notSecure) {
      return true;
    } else {
      try {
        ...
        if (urlAction.nextCheck) {
          ...
          return {
            redirect: urlAction.nextCheck
          };
        }
        return false;
      } catch (e) {
        console.warn("Are you sure you configured the permissions? ", e);
        this.mainservice.showError("Are you sure you configured the permissions?");
      }
    }
  }
  checkVisibility(permissionObject) {
    if (this.mainservice.user && this.mainservice.user.roles && this.mainservice.user.roles.length > 0 && this.mainservice.user.su) return true;else if (this.mainservice.user && !this.mainservice.user.user && this.mainservice.user.roles && this.mainservice.user.roles.length === 0 || this.mainservice.global && this.mainservice.global.notSecure) return true;else {
      if (permissionObject) {
  ...
  comparePermissionObjectWithRoles(object) {
    if (this.mainservice.global && this.mainservice.global.notSecure) {
      ...
      try {
        let check = false;
        if (object[0]) object[0].dcmAcceptedUserRole.forEach(role => {
          if (this.mainservice.user.roles && this.mainservice.user.roles.indexOf(role) > -1) check = true;
        });
        return check;
      } catch (err) {
    ...
    if (this.uiConfig && this.uiConfig.dcmuiAetConfig) {
      try {
        let aetConfig = this.uiConfig.dcmuiAetConfig.filter(config => {
          ...
          if (hasIn_default(config, "dcmAcceptedUserRole") && config.dcmAcceptedUserRole && hasIn_default(this.mainservice, "user.roles") && this.mainservice.user.roles.length > 0) {
...
PermissionService = __decorate([Injectable()], PermissionService);

// src/app/pipes/order-by.pipe.ts
var OrderByPipe = class OrderByPipe2 {
...
export { environment, NG_VALUE_ACCESSOR, NG_VALIDATORS, Validators, ControlContainer, NgControl, FormGroup, NgForm, FormControl, UntypedFormControl, FormGroupDirective, UntypedFormBuilder, FormsModule, ReactiveFormsModule, AppRequestsService, PermissionService, OrderByPipe, MatProgressSpinner, MatProgressSpinnerModule };
