...
export class AppComponent implements OnInit {
    ...
    dcmuiHideClock = false;
    dcmuiPageTitle: string;
    dcmuiHideOtherPatientIDs = false;
    ...
    ngOnInit() {
        ...
        /*        this.appRequests.getDcm4cheeArc().subscribe(res=>{
                    ...
                    console.log("Error on /curalink/ui2/rs/dcm4chee-arc",err);
                });*/
        ...
        try {
            if (url) {
                if (_.hasIn(this.mainservice, 'dcm4cheeArcConfig.deviceNameUrlMap') &&
                    this.mainservice.dcm4cheeArcConfig.deviceNameUrlMap[url] &&
                    this.mainservice.dcm4cheeArcConfig.deviceNameUrlMap[url] !== 'NOT_FOUND') {
    ...
    /*    private compareSavedLanguageWithLanguageInPath() {
            try{
                ...
                const regex = /curalink\/ui2\/(\w{2})\//gm;
                let match;
