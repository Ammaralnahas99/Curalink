...
export class UploadFilesComponent implements OnInit {
    ...
    private setDicomObject(fileTypeOrExt, file0){
        if(fileTypeOrExt === "application/dicom" && this.selectedWebApp && this.selectedWebApp.dcmWebServiceClass && this.selectedWebApp.dcmWebServiceClass.indexOf("CURALINK_ARC_AET") === -1){
            this.neededClassMissing = true;
            this.mainservice.showError($localize `:@@selected_webapp_doesent_have_the_webapp_class:The selected WebApp doesn't have the webapp class ${'CURALINK_ARC_AET'}:webAppServiceClass:`);
        }else{
...
/*    private fileTypeFromExt(fileTypeOrExt:string) {
    ...
    triggerUpload(file,fileIndex, token, seriesInstanceUID, fileTypeOrExt){
        ...
        let url = this.getUrl();
        console.log("url",url);
        //TODO check if the url is corerct for dicom  // POST /curalink/aets/{aet}/rs/study
