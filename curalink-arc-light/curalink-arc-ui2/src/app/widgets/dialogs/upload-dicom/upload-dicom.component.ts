...
" not allowed!`);
                            ...
                            if (xmlHttpRequest.readyState === 4) {
                                if (xmlHttpRequest.status === 200) {
                                    ...
                                    try {
                                        ...
                                        // Extract Study UID
                                        const studyRetrieveUrl = response?.['00081190']?.Value?.[0];
                                        const studyMatch = studyRetrieveUrl?.match(/studies\/([^\/]+)/);
                                        const studyUID = studyMatch ? studyMatch[1] : null;

                                        // Extract Series UID and Instance UID from the instance retrieve URL
                                        const instanceData = response?.['00081199']?.Value?.[0];
                                        const instanceRetrieveUrl = instanceData?.['00081190']?.Value?.[0];
                                        const seriesMatch = instanceRetrieveUrl?.match(/series\/([^\/]+)/);
                                        const instanceMatch = instanceRetrieveUrl?.match(/instances\/([^\/]+)/);
                                        const seriesUID = seriesMatch ? seriesMatch[1] : null;
                                        const instanceUID = instanceMatch ? instanceMatch[1] : null;
                                        ...
                                        if (studyUID && seriesUID && instanceUID) {
                                            const aeTitle = $this.selectedWebApp ? $this.selectedWebApp.dicomAETitle : $this._selectedAe;
                                            // Build WADO URI to view the DICOM image directly
                                            const wadoUrl = `http://localhost:8080/curalink/aets/${aeTitle}/rs/studies/${studyUID}/series/${seriesUID}/instances/${instanceUID}/rendered`;
                                            $this.percentComplete[file.name]['accessionNumber'] = instanceUID.substring(instanceUID.lastIndexOf('.') + 1);
                                    ...
                                    $this.percentComplete[file.name]['showLoader'] = false;
                                    $this.percentComplete[file.name]['value'] = 0;
                                    $this.percentComplete[file.name]['status'] =
    ...
    openStudyWithAccession(accessionNumber: string) {
        navigator.clipboard.writeText(accessionNumber).then(() => {
            // Mark as copied
            Object.keys(this.percentComplete).forEach(key => {
                if (this.percentComplete[key].accessionNumber === accessionNumber) {
                    this.percentComplete[key].copied = true;
                }
            });
            // Open studies page in new tab
            window.open('http://localhost:8080/curalink/ui2/en/#/study/study', '_blank');
        }).catch(err => {
            console.error('Failed to copy accession number:', err);
            // Open anyway
            window.open('http://localhost:8080/curalink/ui2/en/#/study/study', '_blank');
        });
    }
