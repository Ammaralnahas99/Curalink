...
export class AttributeListComponent implements OnInit {
    ...
    init(){
        ...
        if(this.attrs){
            ...
            this.$http.get(url)
                // .map(res => {let resjson; try{ let pattern = new RegExp("[^:]*:\/\/[^\/]*\/auth\/"); if(pattern.exec(res.url)){ WindowRefService.nativeWindow.location = "/curalink/ui2/";} resjson = res.json(); }catch (e){ resjson = [];} return resjson;})
                .subscribe((response) => {
