    ...
    getNoServiceSpecificWebApps(tab: DicomMode) {
        ...
        switch (tab) {
            ...
            case "uwl":
                ...
                break;
            case "diff":
                webServiceClass = 'CURALINK_ARC_AET_DIFF';
...
`;
    ...
    triggerDiff(filterModel, studyWebService: StudyWebService, mode: DicomMode, dicomResponseType: DicomResponseType, file?: File, fileField?: number) {
        if (_.get(studyWebService, "selectedWebService.dcmWebServiceClass") && studyWebService.selectedWebService.dcmWebServiceClass.indexOf("CURALINK_ARC_AET_DIFF") > -1) {
            ...
            return throwError({ error: $localize`:@@webapp_with_service_class_not_found:Web Application Service with the web service class ${'CURALINK_ARC_AET_DIFF'}:webServiceClass: not found!` })
        }
    }
    getDiff(filterModel, studyWebService: StudyWebService, responseType?: DicomResponseType): Observable<any> {
        ...
        if ((batchID || taskID) && url) {
            ...
            return this.getWebAppFromWebServiceClassAndSelectedWebApp(studyWebService, "CURALINK_ARC_AET_DIFF", "CURALINK_ARC_AET_DIFF")
                .pipe(map(webApp => {
    ...
    testAet(url, dcmWebApp: DcmWebApp) {
        return this.$http.get(
            url,//`http://test-ng:8080/curalink/ui2/rs/aets`,
            this.jsonHeader,
            false,
            dcmWebApp
        );
                        ...
                        actions: [
                            ...
                            },
                            {
                                ...
                                showIf: () => {
                                    return j4care.is(options, "appService.global['PDQs']")
                                        ...
                                        && this.selectedWebServiceHasClass(options.selectedWebService, "CURALINK_ARC_AET");
                                }
                            },
                            {
                                icon: {
                            ...
                            },
                            {
                                ...
                                click: (e) => {
                                    ...
                                    return this.selectedWebServiceHasClass(options.selectedWebService, "CURALINK_ARC_AET") && !(_.hasIn(e, 'attrs.77771015'))
                                },
                        ...
                        actions: [
                            {
                                ...
                                click: (e) => {
                                    const accessionNumber = e?.attrs?.['00080050']?.Value?.[0];
                                    if (accessionNumber) {
                                        const url = `https://alhayahscan.nextasolutions.net:8888/viewer/#/viewer?accessionnumber=${accessionNumber}`;
                                        window.open(url, '_blank');
                                    } else {
                                        console.error('No accession number found for this study');
                                        alert('No accession number found for this study');
                                    }
                                },
                                title: 'View in External Viewer',
                                permission: {
                                    id: 'action-studies-download',
                                    param: 'visible'
                                }
                            ...
                            },
                            {
                                ...
                                showIf: (e, config) => {
                                    return this.selectedWebServiceHasClass(options.selectedWebService, "CURALINK_ARC_AET")
                                },
                                permission: {
                                    id: 'action-studies-study',
                                    param: 'edit'
                    ...
                    actions: [
                        {
                            ...
                            click: (e) => {
                                const accessionNumber = e?.attrs?.['00080050']?.Value?.[0];
                                if (accessionNumber) {
                                    window.open(`https://alhayahscan.nextasolutions.net:8888/viewer/?id=portal&password=portal15113030#/viewer?accessionnumber=${accessionNumber}`, '_blank');
                                }
                            },
                            title: 'View DICOM in External Viewer'
                        }
                    ],
                    headerDescription: "View DICOM",
                    pxWidth: 60
                }),
                new TableSchemaElement({
                    type: "value",
                    ...
                    menu: {
                        ...
                        actions: [
                            ...
                            },
                            {
                                ...
                                click: (e) => {
                                    const studyUID = e?.attrs?.['0020000D']?.Value?.[0];
                                    const seriesUID = e?._series?.attrs?.['0020000E']?.Value?.[0];
                                    const instanceUID = e?.attrs?.['00080018']?.Value?.[0];
                                    const aeTitle = options.selectedWebService?.dicomAETitle || 'CURALINK';
                                    if (studyUID && seriesUID && instanceUID) {
                                        const url = `http://localhost:8080/curalink/aets/${aeTitle}/rs/studies/${studyUID}/series/${seriesUID}/instances/${instanceUID}/rendered`;
                                        window.open(url, '_blank');
                                    } else {
                                        console.error('Missing UIDs for DICOM view', { studyUID, seriesUID, instanceUID });
                                    }
                                },
                                id: "instance_view_new_tab",
                                title: 'Open DICOM in new tab',
                                permission: {
                                    id: 'action-studies-download',
                                    param: 'visible'
                                }
                                ...
                                click: (e) => {
                                    ...
                                    const aeTitle = options.selectedWebService?.dicomAETitle || 'CURALINK';
                                    if (studyUID && seriesUID && instanceUID) {
                                        const url = `http://localhost:8080/curalink/aets/${aeTitle}/wado?requestType=WADO&studyUID=${studyUID}&seriesUID=${seriesUID}&objectUID=${instanceUID}&contentType=image/jpeg`;
                                        window.open(url, '_blank');
                                    } else {
                                        console.error('Missing UIDs for DICOM view', { studyUID, seriesUID, instanceUID });
                                    }
    ...
    getModifyStudyWebApp(deviceWebservice: StudyWebService): DcmWebApp {
        if (deviceWebservice.selectedWebService.dcmWebServiceClass.indexOf("CURALINK_ARC_AET") > -1) {
    ...
    rescheduleUPS(workitemUID, deviceWebservice: StudyWebService, model) {
        return this.getModifyUPSUrl(deviceWebservice)
            .pipe(switchMap((url: string) => {
                ...
                return throwError({ error: $localize`:@@error_on_getting_needed_webapp_ups:Error on getting the needed WebApp (with one of the web service classes "CURALINK_ARC_AET" or "UPS_RS")` });
            }))
    }
    ...
    modifyPatientByPk(patientPk: string, patientObject, deviceWebservice: StudyWebService, queued?, batchID?, additionalParams?) {
        return this.getModifyPatientUrl(deviceWebservice)
            .pipe(switchMap((url: string) => {
                ...
                return throwError({ error: $localize`:@@error_on_getting_needed_webapp:Error on getting the needed WebApp (with one of the web service classes "CURALINK_ARC_AET" or "PAM")` });
            }))
    }
    ...
    getModifyPatientWebApp(deviceWebService: StudyWebService): Observable<DcmWebApp> {
        return this.getWebAppFromWebServiceClassAndSelectedWebApp(deviceWebService, "CURALINK_ARC_AET", "PAM");
    }
 ...
 WebApp getModifyPatientUrl`);
    ...
    * */
    ...
    restoreStudy(studyAttr, webService: StudyWebService) {
        let _webApp;
        return this.getWebAppFromWebServiceClassAndSelectedWebApp(webService, "CURALINK_ARC_AET", "REJECT").pipe(map(webApp => {
            _webApp = webApp;
        ...
        /*        return
                    this.$http.post(
                    `${this.studyURL(studyAttr, webApp)}/reject/${rejectionCode}`, //TODO this will work only for internal aets (look this 'CURALINK_ARC_AET' if not found look for this class'REJECT')
                    {},
                    this.jsonHeader
                )}*/
    }
