...
package org.curalink.arc.query.util;

import jakarta.persistence.criteria.*;
import jakarta.persistence.metamodel.CollectionAttribute;
import jakarta.persistence.metamodel.SingularAttribute;
import org.curalink.arc.conf.*;
import org.curalink.arc.entity.*;
import org.curalink.data.*;
import org.curalink.data.PersonName;
import org.curalink.dcmr.AcquisitionModality;
import org.curalink.dict.archive.PrivateTag;
import org.curalink.net.service.QueryRetrieveLevel2;
import org.curalink.soundex.FuzzyStr;
import org.curalink.util.DateUtils;
...
public class QueryBuilder {
    ...
    private <Z, X> boolean orderPersonName(
            From<Z, X> entity, SingularAttribute<X, org.curalink.arc.entity.PersonName> attribute,
            OrderByTag orderByTag, List<Order> result) {
        return orderPersonName(joinPersonName(entity, attribute), orderByTag, result);
    }

    private <Z, X> Path<org.curalink.arc.entity.PersonName> joinPersonName(
            From<Z, X> entity, SingularAttribute<X, org.curalink.arc.entity.PersonName> attribute) {
        return (Path<org.curalink.arc.entity.PersonName>) entity.getJoins().stream()
                .filter(j -> j.getAttribute().equals(attribute))
                .findFirst()
                .orElseGet(() -> entity.join(attribute, JoinType.LEFT));
    }

    private boolean orderPersonName(Path<org.curalink.arc.entity.PersonName> personName,
            OrderByTag orderByTag, List<Order> result) {
    ...
    public List<Predicate> studyPredicates(CommonAbstractCriteria criteria,
        ...
        studyLevelPredicates(predicates, criteria, study, keys, queryParam, QueryRetrieveLevel2.STUDY, showInstancesRejectedByCodes);
        return predicates;
    }
    ...
    public <T> List<Predicate> seriesPredicates(CommonAbstractCriteria criteria,
        ...
        studyLevelPredicates(predicates, criteria, study, keys, queryParam, QueryRetrieveLevel2.SERIES, showInstancesRejectedByCodes);
        seriesLevelPredicates(predicates, criteria, study, series, keys, queryParam, QueryRetrieveLevel2.SERIES, showInstancesRejectedByCodes);
        return predicates;
    }
    ...
    public List<Predicate> instancePredicates(CommonAbstractCriteria criteria,
        ...
        studyLevelPredicates(predicates, criteria, study, keys, queryParam, QueryRetrieveLevel2.IMAGE, showInstancesRejectedByCodes);
        seriesLevelPredicates(predicates, criteria, study, series, keys, queryParam, QueryRetrieveLevel2.SERIES, showInstancesRejectedByCodes);
    ...
    private <Z> Predicate onlyWithStudiesPredicate(CommonAbstractCriteria criteria, From<Z, Patient> patient, QueryParam queryParam) {
        if (queryParam.getAEAccessControlIDs().length == 0)
        ...
        return cb.exists(sq.select(study).where(cb.and(
                accessControlPredicate(study, queryParam.getAEAccessControlIDs()),
    ...
    private <Z> void studyLevelPredicates(List<Predicate> predicates, CommonAbstractCriteria criteria,
        ...
        accessControl(predicates, study, queryParam.getAEAccessControlIDs());
    ...
    private void upsLevelPredicates(List<Predicate> predicates, CommonAbstractCriteria criteria,
        ...
        if (scheduledHumanPerformersSequence != null) {
            ...
            personName(predicates, criteria, ups, UPS_.humanPerformerName,
    ...
    private <T, Z, X> void personName(List<Predicate> predicates, CommonAbstractCriteria criteria, From<Z, X> patient,
                                      SingularAttribute<X, org.curalink.arc.entity.PersonName> attribute, String value,
                                      QueryParam queryParam) {
        if (!isUniversalMatching(value)) {
    ...
    private void literalMatch(List<Predicate> predicates, Path<org.curalink.arc.entity.PersonName> qpn,
    ...
    private <T> void fuzzyMatch(List<Predicate> predicates, CommonAbstractCriteria criteria, Path<org.curalink.arc.entity.PersonName> qpn,
    ...
    private <T> void fuzzyMatch(List<Predicate> predicates, CommonAbstractCriteria criteria,
                                Path<org.curalink.arc.entity.PersonName> qpn, PersonName pn, PersonName.Component c, QueryParam param) {
    ...
    private <T> void fuzzyMatch(List<Predicate> predicates, CommonAbstractCriteria criteria,
                                Path<org.curalink.arc.entity.PersonName> qpn, PersonName.Component c, int partIndex, String name,
                                QueryParam param) {
